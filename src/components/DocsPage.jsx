        ...
        blocks: [
          {
            id: 'api-trending',
            language: 'http',
            code: `GET ${docsMeta.tmdbApiBase}/trending/all/day?api_key=YOUR_KEY&language=en-US`,
          },
          {
            id: 'api-movie-embed',
            language: 'html',
            code: `<iframe src="${docsMeta.playerBase}/movie/1078605?color=e50914&autoPlay=true" allowfullscreen></iframe>`,
          },
        ],
      },
      {
        ...
        blocks: [
          {
            id: 'auth-config',
            language: 'js',
            code: `const firebaseConfig = ${JSON.stringify(docsMeta.firebaseConfig, null, 2)};`,
          },
        ],
      },
      {
        ...
        content: Object.entries(docsMeta.errorDefinitions).map(
          ([code, definition]) =>
            `${code}: ${definition.title}. ${definition.message} Fix: ${definition.help}`,
        ),
        blocks: [],
      },
      {
        ...
        blocks: [
          {
            id: 'env-snippet',
            language: 'bash',
            code: `VITE_TMDB_PROXY_BASE=${docsMeta.tmdbProxyBase}
...
VITE_FIREBASE_APP_ID=${docsMeta.firebaseConfig.appId}`,
  ...
  const filteredSections = sections.filter((section) => {
    const haystack = [
      ...
      section.summary,
      ...section.content,
      ...section.blocks.map((block) => block.code),
    ]
      .join(' ')
      .toLowerCase();
            ...
            >
              ...
              <div className="mt-5 space-y-3">
                {section.content.map((line) => (
                  <div
                    key={line}
                    className="rounded-[22px] border border-white/10 bg-white/[0.03] px-4 py-3 text-sm leading-7 text-white/68"
                  >
                    {line}
                  </div>
                ))}
